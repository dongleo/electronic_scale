...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    if (notifyCallbackId) {
        ...
        NSArray *result = [self marshall: data];
        
        int code = [(NSNumber *)[result objectAtIndex: 1] intValue];
        NSLog(@"didUpdateValueForCharacteristic, %@, %d, %d, %hhd", result, code, LOCKED_FLAG, code == LOCKED_FLAG);
        if (code == LOCKED_FLAG) {
            NSLog(@"didUpdateValueForCharacteristic, %@", result);
            CDVPluginResult *pluginResult = nil;
